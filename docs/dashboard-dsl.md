...
- **The lane cap belongs to the WIDGET, not to your SQL (0.23.0).** `limit`
  ...
  construction the sparse ones — measured on a live ADF estate over a 30-day
  window, a run wall went **+11%** (4,124 → 4,564 folded rows, 74 lanes) and an
  estate-wide step wall **3.3×** (4,119 → 13,662, 321 lanes). Read the RATIOS,
  not the counts: that estate held about five days of history inside the
  30-day window, so the absolute figures are its own and the multiple is what
  travels — and the multiple is set by how long-tailed your lane distribution
  is. Fold with `{{ bucket }}` (above) and that is the shape of the bill;
  without the fold it is the raw row count, and the trade may not be worth it.
